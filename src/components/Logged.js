...
const Logged = ({logout, user, appType}) => {
    ...
    const [chats] = useRequest({initialUrl:`http://${localStorage.getItem('Authorization')}/api/chat/auth/getChats?pageSize=3`, initialValue:[], isAuth: true, fetchOnMount:true, callback:setCurrentList})
    const [orders, fetchOrders] = useRequest({initialUrl: `http://${localStorage.getItem('Authorization')}/api/orders/auth/getOrders`, initialValue:[], isAuth: true})
